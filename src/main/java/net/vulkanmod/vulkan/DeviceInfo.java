...
public class DeviceInfo {
    ...
    public DeviceInfo(VkPhysicalDevice device) {
        ...
        try (MemoryStack stack = MemoryStack.stackPush()) {
            ...
            // Much More Robust VK 1.2 Check, uses the actual/real driver version instead of the Instance ver
    ...
    private static String decodeDvrVersion(int v, int i) {
        return switch (i) {
            ...
            case (0x1022) -> decodeAmdVersion(v); // AMD
            case (0x8086) -> decodeIntelVersion(v); // Intel
            default -> decodeDefVersion(v); // Either AMD or Unknown Driver Encoding Scheme
        };
    }
    ...
    private static String decodeAmdVersion(int v) {
        return decodeDefVersion(v);
    }
    ...
    private static String decodeIntelVersion(int v) {
        return (glfwGetPlatform() == GLFW_PLATFORM_WIN32) ? (v >>> 14) + "." + (v & 0x3fff) : decodeDefVersion(v);
    }
	...
	public static int getGPUApiVer() {	
		...
		var bshift = (properties.apiVersion() >>> 12 & 0x3FF);

		System.out.println("o: " + orig + ", bs: " + bshift);
